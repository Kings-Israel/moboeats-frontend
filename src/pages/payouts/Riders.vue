                ...
                <table class="table-auto w-full dark:text-slate-300">
                  ...
                  <tbody class="text-sm divide-y divide-slate-200 dark:divide-slate-700">
                    <tr v-for="payout in payouts.data" :key="payout.id">
                      ...
                      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="text-left">{{payout.payable.email}}</div>
                      </td>
                      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="text-left font-medium flex w-fit px-2 py-1 rounded-md text-green-500">{{formatValue(payout.amount, payout.payable.country)}}</div>
                      </td>
                      <td class="px-2 py-3 whitespace-nowrap w-px flex gap-1">
                        <router-link class="btn bg-indigo-500 hover:bg-indigo-600 text-white btn-sm" :to="{ name: 'rider-details', 'params': { id: payout.payable.id, type: 'rider' }}">View</router-link>
                      </td>
                    </tr>
                  </tbody>
                </table>
...
export default {
  ...
  setup() {
    ...
    watch(search, async (newSearch, oldQuestion) => {
      $http.get('/admin/payouts/riders?search='+newSearch)
        .then(response => {
          payouts.value = response.data.data.payouts
        })
    })
