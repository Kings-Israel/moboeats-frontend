                    ...
                    <div class="px-2">
                        <div class="w-full flex gap-2">
                            <div v-for="user in user_data" :key="user.key">
                                <label class="block text-sm font-medium mb-1" :for="user.key">{{ user.label }}</label>
                                <input :id="user.key" class="w-full rounded-md border-2 border-slate-400 text-slate-900" type="text" v-model="user.model" />
                            </div>
                        </div>
                        <div class="w-full flex gap-2">
                        ...
                        <h2 class="my-2 text-slate-700 font-bold text-lg">Documents</h2>
                        <div class="w-full flex flex-wrap gap-2">
                            <div v-for="document in documents" :key="document.key">
                                <label class="block text-sm font-medium mb-1" :for="document.key">{{ document.label }}</label>
                                <input :id="document.key" :accept="document.accept" v-on:change="onFileChange" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" type="file" autoComplete="off" />
                            </div>
                        </div>
                    </div>
...
export default {
  ...
  setup() {
    ...
    const onSubmit = async () => {
        ...
        await $http.post('/admin/rider/store', formData)
            .then((res) => {
