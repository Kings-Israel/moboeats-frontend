      ...
      <main class="grow">
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
          ...
          <div class="grid grid-cols-12 gap-6">
            <div class="col-span-full xl:col-span-4">
              ...
              >
                ...
                >
                  ...
                  <button
                    v-if="
                      user_roles.includes('restaurant') ||
                      user_roles.includes('restaurant employee')
                    "
                    class="bg-[#F7C410] hover:bg-[#4f441e] p-1 text-white font-semibold rounded-md px-3 transition duration-150 ease-in-out"
                    @click="editPartnerAdmin = true"
                  >
                    Edit User
                  </button>
                </header>
                <div class="flex flex-col space-y-2 p-3">
                  <h1
                    class="flex gap-2 font-semibold text-slate-800 dark:text-slate-100"
                  >
                  ...
                  >
                    ...
                    ><strong>{{
                      moment(user.created_at).format("Do MMMM Y")
                    }}</strong>
                  </h1>
                  <h1
                    v-if="user_roles.includes('orderer')"
                    class="flex gap-2 font-semibold text-slate-800 dark:text-slate-100"
                  >
                  ...
                  <h1
                    v-if="user_roles.includes('restaurant')"
                    class="flex gap-2 font-semibold text-slate-800 dark:text-slate-100"
                  >
                    <span>Total Branches:</span
                    ><strong>{{ user.restaurants_count }}</strong>
                  </h1>
                  <h1
                    v-if="user_roles.includes('restaurant')"
                    class="flex gap-2 font-semibold text-slate-800 dark:text-slate-100"
                ...
                >
                  <modal-action
                    :id="'addMenu'"
                    :modal-open="editPartnerAdmin"
                    @close-modal="editPartnerAdmin = false"
                    :add-class="'max-w-4xl'"
                  >
                    ...
                    <form
                      @submit.prevent="updatePartnerAdmin()"
                      ...
                      <div class="space-y-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            class="form-input w-full rounded-lg"
                            type="tel"
                            v-model="editEmployeePhoneNumber"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="file"
                            name="addAvatar"
                            v-on:change="selectEmployeeAvatar"
                            id=""
                        ...
                        <div class="block md:flex mt-4">
                          <label
                            class="my-auto block text-sm font-medium mb-1"
                            for="profile_picture"
                            >Change Partner Type to:</label
                          >
                            ...
                            <div class="px-3">
                              ...
                              <label class="flex items-center">
                                <input
                                  type="radio"
                                  name="menu-active"
                                  v-model="editEmployeeType"
                                  value="both"
                                  class="form-radio"
                                />
                                <span class="text-sm font-bold ml-2"
                                  >Restaurant and Grocery Shop</span
                                >
                              </label>
                              <!-- End -->
                            </div>
              ...
              <div
                v-if="user_roles.includes('rider') && rider_profile"
                class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 h-fit"
              >
                <header
                ...
                <div class="flex justify-between gap-1">
                  <div class="flex flex-col space-y-2 p-2">
                    <h1
                      class="flex gap-1 font-semibold text-slate-800 dark:text-slate-100"
                    >
                      <span>Vehicle Type:</span
                      ><strong>{{ rider_profile.vehicle_type }}</strong>
                    </h1>
                    <h1
                      class="flex gap-1 font-semibold text-slate-800 dark:text-slate-100"
                    >
                      ...
                      ><strong>{{
                        rider_profile.vehicle_license_plate
                      }}</strong>
                    </h1>
                    <h1
                      class="flex gap-1 font-semibold text-slate-800 dark:text-slate-100"
                    >
                      <span>Registered On:</span
                      ><strong>{{
                        moment(rider_profile.created_at).format("Do MMMM Y")
                      }}</strong>
                    </h1>
                  </div>
                  <img
                    :src="rider_profile.profile_picture"
                    alt=""
                    class="w-20 h-20 rounded-full object-cover mx-1"
                  />
                </div>
                <div class="p-2 flex justify-between">
                  ...
                  >
                    ...
                    ><strong
                      :class="
                        'p-1 px-2 rounded-md ' +
                        resolveRiderStatus(rider_profile.status)
                      "
                      >{{ rider_profile.status }}</strong
                    >
                  </h1>
                  <div class="flex gap-1">
                    <form
                      @submit.prevent="
                        updateRiderStatus(rider_profile.uuid, 'approved')
                      "
                      v-if="
                        rider_profile.status == 'Pending' ||
                        rider_profile.status == 'Denied'
                      "
                    >
                      <button
                    ...
                    <button
                      v-if="rider_profile.status == 'Pending'"
                      class="bg-red-500 text-white rounded-md px-2"
                      @click="updateRiderStatusModal = true"
                    >
                      Reject
                    </button>
                    <modal-action
                      v-if="rider_profile.status == 'Pending'"
                      :id="'udpateRiderStatus'"
                      :modal-open="updateRiderStatusModal"
                      @close-modal="updateRiderStatusModal = false"
                      :add-class="'max-w-4xl'"
                    >
                      <!-- Add/Edit Menu -->
                      <form
                        ...
                        @submit.prevent="
                          updateRiderStatus(rider_profile.uuid, 'denied')
                        "
                      >
                        <div>
                          <label
                          ...
                          <textarea
                            id=""
                            class="form-input w-full min-h-full rounded-lg border-2 border-slate-400"
                            rows="4"
                ...
                <h1
                  class="flex gap-1 font-semibold text-slate-800 dark:text-slate-100 p-2"
                  v-if="rider_profile.status == 'Denied'"
                >
                  <span>Rejection Reason: </span
                  ><strong>{{ rider_profile.rejection_reason }}</strong>
                </h1>
              </div>
              <div
                v-if="user_roles.includes('rider') && !rider_profile"
              ...
              >
                ...
                >
                  <h2
                    class="font-semibold text-red-800 dark:text-red-500 text-center"
                  >
                    Rider Profile Not Created
                  </h2>
                </header>
              </div>
            </div>
                    ...
                    >
                      ...
                      <tr v-for="order in orders" :key="order.id">
                        ...
                        <td class="p-2">
                          <div
                            :class="
                              'text-center ' + resolveOrderStatus(order.status)
                            "
                          >
                            {{ order.status }}
                          </div>
                        ...
                        <td class="p-2">
                          <div class="text-center">
                            {{ moment(order.created_at).format("Do MMM Y") }}
                          </div>
                        </td>
                ...
                <div class="overflow-x-auto">
                  <table class="table-auto w-full dark:text-slate-300">
                    ...
                    >
                      <tr>
                        <th class="p-2">
                          <div class="font-semibold text-left">Branches</div>
                        </th>
                        <th class="p-2">
                          <div class="font-semibold text-center">
                            Created On
                  ...
                  <div class="mt-8">
                    <PaginationClassic
                      @change-page="changeRestaurantsPage"
                      :next_page="nextRestaurantsPageUrl"
                      :prev_page="prevRestaurantsPageUrl"
                      :from="restaurantsFrom"
                      :to="restaurantsTo"
                      :total_items="restaurantsTotalItems"
                    />
                    <!-- <PaginationNumeric @change-page="changePage" :next_page="nextOrdersPageUrl" :prev_page="prevOrdersPageUrl" :from="from" :to="to" :total_items="totalItems" :links="ordersPagesLinks" /> -->
                  </div>
                </div>
            ...
            <div
              v-if="user_roles.includes('rider')"
              class="col-span-full xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700"
            >
              <header
                class="px-5 py-2 border-b border-slate-100 dark:border-slate-700"
              ...
              <div class="p-3">
                ...
                <div class="overflow-x-auto">
                      ...
                      <tr v-for="delivery in deliveries" :key="delivery.id">
                        <td class="p-2">
                          <div class="">
                            <!-- TODO:Add Link to View Order -->
                            <div
                              class="text-slate-800 dark:text-slate-100 m_upper"
                            >
                              {{ getOrderId(delivery) }}
                            </div>
                          </div>
                        ...
                        <td class="p-2">
                          <div
                            :class="
                              'text-center ' +
                              resolveOrderStatus(delivery.status)
                            "
                          >
                            {{ delivery.status }}
                          </div>
                        ...
                        <td class="p-2">
                          <div class="text-center">
                            {{ moment(delivery.created_at).format("Do MMM Y") }}
                          </div>
                        </td>
                        <td class="p-2 flex justify-center">
                          <button
                  ...
                  <div class="mt-8">
                    <PaginationClassic
                      @change-page="changeDeliveriesPage"
                      :next_page="nextDeliveriesPageUrl"
                      :prev_page="prevDeliveriesPageUrl"
                      :from="deliveriesFrom"
                      :to="deliveriesTo"
                      :total_items="deliveriesTotalItems"
                    />
                    <!-- <PaginationNumeric @change-page="changePage" :next_page="nextOrdersPageUrl" :prev_page="prevOrdersPageUrl" :from="from" :to="to" :total_items="totalItems" :links="ordersPagesLinks" /> -->
                  </div>
                </div>
                      ...
                      <div class="grow p-5">
                        <header>
                          ...
                          <div class="flex justify-center items-center">
                            ...
                            ><br />
                            <span class="mx-1"
                              >(Deliveries: {{ earnings_data.order_earnings }},
                              Tips: {{ earnings_data.tip_earnings }})</span
                            >
                          </div>
                        </header>
                      </div>
...
export default {
  ...
  setup() {
      ...
      $http.get(url).then((response) => {
        ...
        if (user.value) {
          ...
          if (user_roles.value.includes("rider")) {
            ...
            nextDeliveriesPageUrl.value =
              response.data.data.deliveries.next_page_url;
            lastDeliveriesPageUrl.value =
              response.data.data.deliveries.last_page_url;
            prevDeliveriesPageUrl.value =
              response.data.data.deliveries.prev_page_url;
    ...
    const updateRiderStatus = (id, status) => {
      $http
        .post("admin/users/rider/" + id + "/status/update", {
          status: status,
    ...
    function changeRestaurantsPage(page) {
      $http.get(page + "&user=restaurants").then((response) => {
        nextRestaurantsPageUrl.value =
          response.data.data.restaurants.next_page_url;
        lastRestaurantsPageUrl.value =
          response.data.data.restaurants.last_page_url;
        prevRestaurantsPageUrl.value =
          response.data.data.restaurants.prev_page_url;
    ...
    function changeOrdersPage(page) {
      $http.get(page + "&user=orderer").then((response) => {
    ...
    onMounted(() => {
      ...
      switch (router.params.type) {
        ...
        case "restaurant-admin":
          url =
            "/admin/users/restaurant-admin/" + router.params.id + "/details";
          break;
        case "rider":
